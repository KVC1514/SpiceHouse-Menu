...
import { db } from "../../main";
import { collection, onSnapshot, deleteDoc, doc } from "firebase/firestore"; // Importing necessary Firestore methods
...
function Menu() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this?")) {
      try {
        setOpen(false);
        const docRef = doc(db, "users", id); // Create document reference
        await deleteDoc(docRef); // Delete the document
        setUsers(users.filter((user) => user.id !== id)); // Remove from UI
        console.log("User deleted successfully"); // Debugging message
      } catch (err) {
    ...
    <AppLayout>
                    ...
                    <Card style={{ width: "300px", height: "400px" }}>
                      <Card.Content>
                        <Image
                          ...
                          onError={(e) => {
                            e.target.src = "path/to/placeholder/image.png"; // Placeholder image if image load fails
                          }}
                        />
      ...
      {open && (
        <ModalComp
          ...
          setOpen={setOpen}
          {...user}
          handleDelete={handleDelete} // Pass the delete handler to ModalComp
        />
      )}
    </AppLayout>
  );
